 ...
 * [S. Koenig, 2002]
 */
void Dstar::init(int sX, int sY, int sZ, int gX, int gY, int gZ) {
 ...
 * As per [S. Koenig, 2002]
 */
void Dstar::updateCell(int x, int y, int z, double val) {
...
void Dstar::getSucc(state u,list<state> &s) {
  ...
  if (occupied(u)) return;
    ...
    /*
    std::cout<<u.x<<" "<<u.y<<" "<<u.z<<" with successors \n";
    for(const auto l: s){
 ...
 */
void Dstar::updateStart(int x, int y, int z) {
 ...
 */
void Dstar::updateGoal(int x, int y, int z) {
  ...
  for (kk=toAdd.begin(); kk != toAdd.end(); kk++) {
      updateCell(kk->first.x, kk->first.y, kk->first.z, kk->second);
  }
